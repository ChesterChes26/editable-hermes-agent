...
- **Implementing code changes (even diagnostic instrumentation) without user agreement during collaborative investigation** — when the user is driving the debugging session, they own the code. Proposing fixes before root cause is confirmed wastes time; modifying code without asking erodes trust. State your hypothesis, ask for permission, then act.
