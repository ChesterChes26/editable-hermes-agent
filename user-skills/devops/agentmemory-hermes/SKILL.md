...
| hook | 触发 | 做什么 |
|------|------|--------|
| system_prompt_block | session 启动 | 仅 health-check（GET /agentmemory/sessions），失败返回 `status="UNAVAILABLE"` 信号；成功返回空——不注入 context（只存不取模式） |
| prefetch | 每次 LLM 调用前 | 已禁用（只存不取模式），直接返回 `""` |
| queue_prefetch | 每次 LLM 调用前 | 已禁用（只存不取模式），no-op |
...
| on_pre_compress | 上下文压缩前 | 已禁用（只存不取模式），no-op |
...
**system_prompt_block 健康检查（2026-06-29）：** 只存不取模式下，`system_prompt_block()` 不再调用 `/agentmemory/context`——仅跑 `GET /agentmemory/sessions` 做轻量 health check。用 sessions 端点而非 smart-search 是为了避免不必要的搜索索引负载。如果不可达，返回 `status="UNAVAILABLE" agentmemory not reachable`，被 `memory_manager` 包在 `<memory-context>` 中注入 system prompt。如果可达则返回空——不注入任何 context。成本：每次 session 初始化一次 GET sessions 调用。

**⚠ 验证陷井：不要靠症状推断 `_api()` 的行为——读源码。** `_api()` 的返回值判定链（`__init__.py:156-172`）不是直觉型的：空 HTTP 200 body → `resp.read().decode()` 返回 `""` → `json.loads("")` 抛出 `JSONDecodeError` → 被 `except` 捕获 → 返回 `None` → `if not _api(...)` 为 True → 触发 UNAVAILABLE。如果仅观察症状（system prompt 无 UNAVAILABLE + agentmemory 间歇返回空）就断定"空响应被解析为 truthy"，结论会完全相反——实际上空 body 会正确触发 UNAVAILABLE。正确的验证顺序：(1) 读 `_api()` 源码 156-172 行确认异常捕获范围，(2) 实际 curl 端点看返回什么，(3) 结合两者得出结论。跳过第 1 步直接推断 = 必错。

## 只存不取模式（2026-06-29）

当注入的 summaries 质量差（测试垃圾、泛泛摘要）或产生 LLM 推理干扰时，可以将 agentmemory 切换为"只存不取"——自动写入保持，自动读取全部关闭。

### 效果

| 操作 | 之前 | 只存不取 |
|------|------|---------|
| system_prompt_block | 加载 `/context` 注入 system prompt | 仅 health check（GET /sessions），不注入 |
| prefetch | 每轮 smart-search Top-5 注入 prompt | 直接返回 `""` |
| queue_prefetch | 后台 smart-search 3 条 | no-op |
| sync_turn | 记录对话 | 不变 |
| memory_save/recall/search | 手动调用 | 不变 |
| on_pre_compress | 压缩前注入 context | no-op |

### 实施方法

直接修改 `plugins/agentmemory/__init__.py`：

- `system_prompt_block()` — 删除 `/context` 调用，仅留 health check
- `prefetch()` — return `""`
- `queue_prefetch()` — return（no-op）

### 恢复自动注入

回退上述三处修改，或 git checkout 原版 `__init__.py`。改后需 `hermes gateway restart && hermes gateway run &`。

### 注意

- manual 工具（memory_save/recall/search）不受影响，agent 仍可在需要时主动查询
- sync_turn 继续记录对话，所有数据仍在 agentmemory 中积累
- 将来 summaries 质量改善后可随时恢复自动注入
...
**⚠ HTTPError 陷井（2026-06-29 踩坑）：** 即使实测了 curl 返回 404，Python 代码仍有坑。`urllib.request.urlopen` 遇到任何非 2xx 状态码都会抛 `HTTPError`，而非正常返回 response object。`container_http_healthy()` 原来用 `except Exception: return False` 吞了 404 的 `HTTPError`，导致 iii-engine 完全健康时被误判为 HTTP-stuck → watchdog 反复 `docker restart` → 永远走不到 spawn worker 分支。

**症状：** 手动 curl `GET /` 返回 404（正常），但 watchdog 输出 "Container HTTP-stuck — restarting..." → "Container restart failed"（因为容器已经在跑，restart 后状态不变）。
...
# ...
    ...
    return e.code < 500  # 404 = healthy, 500+ = unhealthy
except Exception:
    return False
```

验证修复后 watchdog 输出："Worker down, starting... → Worker online."
...
## Session 启动时主动验证（Agent 工作流）

**自动检测（2026-06-29）：** `system_prompt_block()` 已内置 health check（GET /agentmemory/sessions）。agent 无需手动验证——每轮对话的 system prompt 中 `<memory-context>` 块会直接显示 `status="UNAVAILABLE"`。看到这个标记即知 agentmemory 挂了。只存不取模式下，service 在线时该块为空（不注入 context），正常状态无额外 token 开销。

**手动降级方案：** 当 system prompt 显示 UNAVAILABLE 或 agent 需要主动修复后再验证时：

```bash
# 一行确诊（比 curl 快，比 MCP 工具更可靠——MCP 工具可能返回空 JSON 而非报错）
...
|--------|------|------|
| 200 + JSON body | 正常 | 继续 |
| 404 | Worker 未注册 | 跑 watchdog：`python ~/AppData/Local/hermes/scripts/agentmemory-watchdog.py` |
| 000 / timeout | 容器挂了或 HTTP-stuck | 同上，watchdog 会自动 docker start/restart |
| Connection refused | 容器未启动 | 启动 Docker Desktop 或手动 `docker start agentmemory-iii-engine-1` |

**如果健康检查失败：** 不要等 watchdog 下一分钟轮回——立即手动跑一次 watchdog script，然后告诉用户 "agentmemory 刚恢复，本 session 的早期对话可能未被记录"。
