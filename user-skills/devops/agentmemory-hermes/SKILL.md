...
每分钟运行的 watchdog 脚本，三层诊断避免误判：

| 检测 | 条件 | 动作 |
|------|------|------|
| worker 正常 | smart-search 返回 200 | 立即退出 |
| worker 挂了（容器健康） | TCP reachable + 容器 running | `npx` 重启 worker（`CREATE_NO_WINDOW` 无弹窗） |
| 容器 TCP-stuck | 容器 running 但 TCP 不通 | `docker restart`（无弹窗） |
| 容器挂了 | docker ps 找不到 | `docker start` + 等 worker 重连 |

关键改进：容器级问题用 `docker restart` 而非 `npx`——不会触发 Windows cmd 闪窗。所有子进程加 `creationflags=subprocess.CREATE_NO_WINDOW`。

```bash
# 1. 创建 watchdog 脚本
cat > ~/AppData/Local/hermes/scripts/agentmemory-watchdog.py << 'EOF'
"""agentmemory worker watchdog — auto-restart if dead."""
import os, subprocess, sys, time
from urllib.request import Request, urlopen

API_URL = os.environ.get("AGENTMEMORY_URL", "http://localhost:3111")
HEALTH_URL = f"{API_URL}/agentmemory/smart-search"
NPX = r"C:\Program Files\nodejs\npx.cmd"  # Windows 绝对路径

def worker_alive():
    try:
        req = Request(HEALTH_URL, data=b'{"query":"h","limit":1}',
...
subprocess.Popen([NPX, "@agentmemory/agentmemory"], env=env,
...
while time.monotonic() < deadline:
    ...
    if worker_alive():
        ...
        sys.exit(0)
print("[watchdog] Worker did not come online within 60s.")
sys.exit(1)
EOF

# 2. 创建 cronjob（每分钟 tick，no_agent=true 直接跑脚本）
