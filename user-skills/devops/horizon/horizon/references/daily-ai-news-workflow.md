...
```bash
cp "$HORIZON_HOME/data/summaries/horizon-YYYY-MM-DD-zh.md" \
...
- **DO NOT check for new run immediately after dispatch.** Subagent needs 2-3 min to initialize session, load tools, start fetch, and write run_id to disk. Checking `hz_list_runs` at <60s yields false negative ("no new run" → "subagent isn't working"). Wait ≥2 min before first check, or `ls data/mcp-runs/` directly (bypasses RunStore memory cache, faster).
