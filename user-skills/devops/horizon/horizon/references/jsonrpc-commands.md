...
Copy-paste templates for calling Horizon MCP tools directly over stdio. All commands assume `cd $HORIZON_HOME` first. Use `printf` (not `echo`) to avoid newline issues with multi-line JSON.
