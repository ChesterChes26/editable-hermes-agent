...
MCP server tools are only available in Hermes when the gateway is running with the server connected. **Critical distinction:** `hz_get_metrics` and `hz_list_runs` are **direct fs** tools — they read JSON/MD from disk and work even when the MCP pipe is broken. They are NOT valid connectivity tests. Use `hz_validate_config` as the **definitive pipe health test** — it goes through `_call_tool` → MCP subprocess → JSON-RPC, confirming the full transport chain (gateway → plugin → subprocess → MCP server). If `hz_validate_config` fails with `"MCP subprocess closed stdout during initialize"`, the plugin's Python lacks Horizon dependencies (see "Gateway Python may differ" pitfall below).

Use this diagnostic escalation ladder — start at the top and only go deeper if the previous step fails:
...
**Horizon runs from its own isolated venv** at `$HORIZON_HOME\.venv\`. It does NOT share the Hermes gateway Python or the hermes-agent venv. The Horizon plugin's `HORIZON_CMD` points to `$HORIZON_HOME\.venv\Scripts\python.exe` — the MCP subprocess uses this Python exclusively.

Dependencies are listed in `pyproject.toml`. Install all at once into the Horizon venv:

```bash
cd $HORIZON_HOME
# Create venv if missing
$EDITABLE_HERMES\.venv\Scripts\python.exe -m venv .venv

# Install Horizon + all deps
.venv\Scripts\pip.exe install -e .
```

Key deps: `feedparser`, `anthropic`, `openai`, `google-genai`, `httpx`, `ddgs`, `beautifulsoup4`, `rich`, `tenacity`, `mcp`.

To verify the venv is self-contained:
```bash
$HORIZON_HOME\.venv\Scripts\python.exe -c "import mcp.server.fastmcp; print('OK')"
$HORIZON_HOME\.venv\Scripts\pip.exe list | grep horizon
# → horizon  0.1.0  $HORIZON_HOME
```

If tools return `HZ_IMPORT_FAILED`, a dependency is missing in the Horizon venv — install it (or all of them with `-e .`).
...
### Multi-profile: runtime plugin ≠ AppData plugin — patch the RIGHT one

**Iron law — first step before any Horizon operation on this machine: `echo $HERMES_HOME`.** hermes-e (`$HERMES_HOME`) and the default Hermes (`%LOCALAPPDATA%\hermes`) are two independent runtimes with separate `plugins/`, `config.yaml`, and gateway processes. Operating on the wrong profile's plugin or config has zero effect on the current session but silently wastes debugging time. Always confirm which profile you're in before touching any file.

When Hermes runs under a non-default profile (e.g. `HOME=$HERMES_HOME`), the gateway loads plugins from `$(HERMES_HOME)/plugins/`, NOT from `%LOCALAPPDATA%/hermes/plugins/`. Both directories may contain a `horizon` plugin with identical-looking `__init__.py` files — but patching the wrong one has zero effect.

**Symptoms:**
- You patch `__init__.py` at `AppData/Local/hermes/plugins/horizon/` and restart gateway
- The change has no effect — same error persists
- The runtime plugin at `hermes-e-data/plugins/horizon/__init__.py` still has the old code
- `hz_get_metrics` works (direct fs) but pipe tools fail (MCP subprocess)

**Diagnosis — which plugin does the gateway actually load?**
```bash
# Check which Hermes home the gateway uses
echo $HERMES_HOME
# → $HERMES_HOME  (runtime plugin lives here)

# Compare the two copies
diff \
  ~/AppData/Local/hermes/plugins/horizon/__init__.py \
  /e/new_workspace/hermes-e-data/plugins/horizon/__init__.py
```

**Fix — patch the RUNTIME plugin, not the AppData one:**
```bash
# This session's HOME is hermes-e-data → patch HERE:
patch(path="$HERMES_HOME\\plugins\\horizon\\__init__.py", ...)

# NOT here (this is default Hermes profile):
# patch(path="$HOME\\AppData\\Local\\hermes\\plugins\\horizon\\__init__.py", ...)
```

If the two copies have diverged, sync runtime ← source (from `editable-hermes-agent/user-plugins/horizon/`), then apply patches to the runtime copy. The AppData copy belongs to the default profile and should NOT be modified unless working on that profile.
...
~/AppData/Local/hermes/hermes-agent/venv/Scripts/pip.exe install -e $HORIZON_HOME
...
# should show: horizon  0.1.0  $HORIZON_HOME
```
...
### Gateway restart required after plugin file changes — no hot-reload (CRITICAL)

**Plugins are loaded once at gateway startup and cached in memory.** Modifying plugin files on disk (e.g. `__init__.py`, `plugin.yaml`) has zero effect until the gateway process exits and a new one starts. There is no hot-reload mechanism.

This is distinct from "Session stale bytecode" (below) — that pitfall covers agent sessions within a running gateway needing `/reset` after gateway restart. The gateway-level issue is: **the plugin module is imported once and stays in `sys.modules` for the lifetime of the gateway process.**

**Symptoms:**
- Plugin files on disk have been corrected (verified with `diff` / file read)
- `hermes gateway restart` was NOT run after the fix
- Same error persists across `/reset` and `/new` — the gateway is still running the old bytecode
- Only `hermes gateway restart` resolves it

**Fix:**
```bash
hermes gateway restart   # kills old gateway, starts new one → re-imports all plugins from disk
# Then /reset the CLI session
```

**Why this matters for Horizon specifically:** The Horizon plugin's `HORIZON_CMD` path (which Python binary spawns the MCP subprocess) is a module-level variable in `__init__.py`. If the path was wrong at gateway startup, every pipe tool fails with `"MCP subprocess closed stdout during initialize"`. Fixing the file on disk does nothing — the stale `HORIZON_CMD` lives in the gateway process's memory. Only a gateway restart picks up the corrected path.

**Diagnostic pattern when pipe tools fail but disk reads work:**
1. `hz_get_metrics` works → disk read path is fine, plugin is loaded
2. `hz_validate_config` fails → pipe path is broken, MCP subprocess can't start
3. Check `%TEMP%\horizon_stderr.log` for the actual error (`ModuleNotFoundError`, wrong Python, etc.)
4. Fix the root cause (install deps, correct `HORIZON_CMD`, etc.)
5. `hermes gateway restart` → re-test with `hz_validate_config`

**Cleanup before restart — kill zombie MCP subprocesses:**
On Windows, `hermes gateway restart` sometimes leaves orphan MCP server subprocesses. Before restarting, verify:
```bash
# Check for residual Horizon MCP processes
wmic process where "name='python.exe'" get ProcessId,ExecutablePath,CommandLine 2>/dev/null | grep "src.mcp.server"
# Kill them if present: taskkill /F /PID <pid>
```

Two gateway processes running simultaneously (old not fully terminated) is also possible — check with:
```bash
wmic process where "name='pythonw.exe'" get ProcessId,CommandLine 2>/dev/null | grep "gateway"
```

Full session: see `references/gateway-restart-fixes-plugin-2026-07-05.md`.

### Gateway Python may differ from Hermes venv Python — plugin subprocess lacks dependencies

**Note (2026-07-05): This pitfall describes a historical issue. The current setup has Horizon running from its own isolated venv (`$HORIZON_HOME\.venv\`) — the plugin's `HORIZON_CMD` now points to that Python, not `sys.executable`. This pitfall is retained for troubleshooting scenarios where Horizon's own venv is missing or broken.**

The Horizon plugin uses `sys.executable` for `HORIZON_CMD` (`__init__.py:16`). When the Hermes gateway process runs under a Python that is NOT the Hermes venv Python (e.g. a uv-managed `pythonw.exe` at `%APPDATA%/uv/python/...`), the spawned MCP subprocess runs with a Python that lacks `mcp` and other Horizon dependencies — even though the Hermes venv has everything installed.

**Symptoms:**
- All pipe tools (`hz_validate_config`, `hz_run_pipeline`, etc.) return `{"ok": false, "error": "MCP subprocess closed stdout during initialize"}`
- `%TEMP%\horizon_stderr.log` shows `ModuleNotFoundError: No module named 'mcp'`
- Manual test with Hermes venv Python works: `/c/Users/.../hermes-agent/venv/Scripts/python.exe -c "import mcp"` → OK
- `pip show mcp` in Hermes venv confirms it's installed
- But the gateway's Python doesn't have it

**Diagnosis:**
```bash
# 1. Check which Python the gateway process is using
wmic process where "name='pythonw.exe'" get ExecutablePath
# → $HOME\AppData\Roaming\uv\python\...\pythonw.exe  (NOT the venv Python!)

# 2. Verify that Python lacks mcp
/c/Users/admin/AppData/Roaming/uv/python/cpython-3.11-windows-x86_64-none/python.exe -c "import mcp"
# → ModuleNotFoundError

# 3. Verify Hermes venv Python has it
~/AppData/Local/hermes/hermes-agent/venv/Scripts/python.exe -c "import mcp.server.fastmcp; print('OK')"
# → OK
```

**Fix A (PREFERRED) — install Horizon deps into the gateway Python:**

The error message includes the gateway Python path: `CMD=<path>`. Use THAT path — do not guess or hardcode.

```bash
# 1. Identify the gateway Python from the error message:
#    "MCP subprocess closed stdout during initialize (CMD=E:\...\python.exe)"
#    → GATEWAY_PYTHON = E:\...\python.exe

# 2. Check if pip exists in that venv
<GATEWAY_PYTHON> -m pip --version 2>&1
# → "No module named pip" → step 3
# → "pip 24.0 from ..." → skip to step 4

# 3. Bootstrap pip if missing (common in minimal uv-created venvs)
<GATEWAY_PYTHON> -m ensurepip --default-pip

# 4. Install Horizon + all deps
cd <horizon-project-dir>
<GATEWAY_PYTHON> -m pip install -e "<horizon-project-dir>"
```

If the gateway Python is externally-managed (uv Python at `%APPDATA%/uv/python/...`), add `--break-system-packages`. If it's a regular venv (e.g. `editable-hermes-agent\.venv`), the flag is not needed.

**Gateway restart may NOT be needed.** The plugin's `_ensure_proc()` detects dead subprocesses via `poll()` and auto-restarts them. After installing deps, re-invoke the failing pipe tool — the plugin will spawn a fresh subprocess with the now-working Python. Only restart the gateway if `_ensure_proc` has cached a zombie subprocess whose `poll()` still returns `None`.

**Why this is preferred:** It keeps `HORIZON_CMD = [sys.executable, ...]` portable — no hardcoded user-path dependencies in plugin code. The gateway Python (`sys.executable`) is the natural host for plugin subprocess dependencies.

**Fix B (fallback) — hardcode the Hermes venv Python path:**
Only use this when `pip install --break-system-packages` is not available:
```python
# In plugins/horizon/__init__.py, change:
HORIZON_CMD = [sys.executable or "python", "-m", "src.mcp.server"]
# To:
HORIZON_CMD = [r"$HOME\AppData\Local\hermes\hermes-agent\venv\Scripts\python.exe", "-m", "src.mcp.server"]
```

After patching: `hermes gateway restart` then `/reset` the session to reload plugin bytecode.

**When the plugin subprocess is broken, fall back to manual JSON-RPC** (see "stdin closure kills async pipeline" pitfall below for the correct pattern).

### `%TEMP%\horizon_stderr.log` retains old errors — clear it before diagnosis

The plugin opens `horizon_stderr.log` in append mode (`"a"`). Old errors from previous sessions or path migrations persist across gateway restarts. When diagnosing a new failure, always clear it first:

```bash
cmd.exe /c "del %TEMP%\horizon_stderr.log"
```

Then retry the failing tool and check the fresh content. Without clearing, you may chase stale errors (e.g. `ModuleNotFoundError` from a `D:\workspace\` path that no longer exists).
...
### stdin closure kills async pipeline during manual JSON-RPC (CRITICAL for long-running tools)

When issuing `hz_run_pipeline` (or any long-running tool) via manual JSON-RPC piping, **stdin must be kept open** for the entire duration. The MCP server's async `TaskGroup` (`anyio`) cleans up when stdin closes — this closes stdout/write_stream before the tool's response can be sent, producing `anyio.ClosedResourceError`.

**Pattern that FAILS:**
```bash
# stdin closes immediately after printf → async cleanup kills pipeline mid-run
printf '{...initialize...}\n{...tools/call hz_run_pipeline...}\n' | python -m src.mcp.server
# → anyio.ClosedResourceError after ~60-100s (whenever response is ready)
```

**Pattern that WORKS — keep stdin open with sleep:**
```bash
cd $HORIZON_HOME

# sleep N keeps stdin open; use N=1200 (20 min) for full 24h pipeline
(printf '{...initialize...}\n{...tools/call hz_run_pipeline...}\n'; sleep 1200) | \
...
**Why sleep works:** `sleep` holds stdin open without writing. The MCP server continues processing (fetch → score → filter → enrich → summarize) while stdin is still connected. When finished, it writes the JSON-RPC response to stdout, then the process exits naturally.

**Monitoring during long runs:** Since the manual MCP server is separate from the plugin's subprocess, plugin-based read tools (`hz_list_runs`, `hz_get_run_stage`) see a different subprocess and won't find the runs. **Read the run directory directly from disk:**
```bash
ls -la $HORIZON_HOME/data/mcp-runs/<run-id>/
# Look for: raw_items.json → scored_items.json → filtered_items.json → enriched_items.json
```

Full session details: `references/manual-pipeline-bypass-2026-07-05.md`.
...
**Cleanup:**
```bash
rm -rf $HORIZON_HOME/data/mcp-runs/<zombie-run-id>
```
...
### Path migration between environments (skill + plugin sync)

When Horizon moves to a new path, skill `.md` files under `hermes-e-data/skills/` contain hardcoded paths that must be updated.

**Correct workflow** (in order):
1. **Sync user-skills → runtime**: `cp -r editable-hermes-agent/user-skills/devops/horizon hermes-e-data/skills/devops/horizon`
2. **Replace paths in all .md files** (3 variants: forward-slash, backslash, MSYS-style):
   ```bash
   cd hermes-e-data/skills/devops/horizon
   find . -name "*.md" -exec sed -i \
     -e 's|D:/workspace/AI-research/Horizon|$HORIZON_HOME|g' \
     -e 's|D:\\\\workspace\\\\AI-research\\\\Horizon|E:\\\\new_workspace\\\\Horizon|g' \
     -e 's|/d/workspace/AI-research/Horizon|/e/new_workspace/Horizon|g' \
     {} \;
   ```
3. **Also check sibling skills** that reference the old path (e.g. `path-sync`)
4. **Switch Horizon branch** (e.g. `git checkout chester-edit && git rebase main`)
5. **Restart gateway** and test with `hz_get_metrics`

**Pitfall — do NOT modify plugin code as a workaround.** If `hz_get_metrics` still returns the old `runs_root` after fixing all `.md` files, do NOT change `HORIZON_CWD` in `plugins/horizon/__init__.py` or create filesystem junctions. The stale path is still hiding in an unfound source file — keep searching. Modifying the plugin or the filesystem masks the real problem.

**Pitfall — nested skill directories.** The skill directory may contain a nested copy (e.g. `hermes-e-data/skills/devops/horizon/horizon/SKILL.md`). Check for nested dirs after syncing.

**Common stale-path locations:**
- `hermes-e-data/skills/devops/horizon/SKILL.md`
- `hermes-e-data/skills/devops/horizon/references/*.md`
- `hermes-e-data/skills/devops/horizon/horizon/SKILL.md` (nested)
- `hermes-e-data/skills/devops/path-sync/SKILL.md` (sibling skill)
- `editable-hermes-agent/user-skills/devops/horizon/` (source of truth, sync to runtime; do NOT edit source)

**Path variants to catch:** `D:/workspace/...`, `D:\\\\\\\\workspace\\\\\\\\...`, `/d/workspace/...` — all need to be replaced. Verify with: `grep -r "OLD_PATH_PATTERN" hermes-e-data/skills/ hermes-e-data/plugins/ Horizon/src/ --include="*.py" --include="*.md"`

**Pitfall — leftover NTFS junction at old path survives as ghost reference (幽灵路径).** After path migration, an NTFS junction created as a temporary workaround may survive and cause the old path to reappear in diagnostics. `grep` over file contents alone will NOT find it — it lives at the filesystem level, not in any text file. When any code traverses through the junction, the old prefix leaks into logs, tool output, and error messages.

**Detection — check for reparse point attribute:**
```bash
ls -la "D:/workspace/AI-research/Horizon" 2>&1
# Junction: shows "lrwxrwxrwx ... -> /e/new_workspace/Horizon"
# Regular dir (after fsutil stripped reparse point): shows "drwxr-xr-x"

# Definitive check: GetFileAttributesW returns FILE_ATTRIBUTE_REPARSE_POINT (0x400)
python -c "
import ctypes
...
**Removal — `cmd //c rmdir` DOES NOT WORK (it follows the junction into the target):**
```bash
# WRONG — cmd follows the junction, rmdir runs inside the target
cmd //c "rmdir D:\\workspace\\AI-research\\Horizon"

# RIGHT — use Python shutil.rmtree (handles junction + residual contents)
python -c "import shutil; shutil.rmtree(r'D:\workspace\AI-research\Horizon')"

# Then clean up the empty parent directory
rmdir "D:/workspace/AI-research"
```

**After removal — gateway restart is REQUIRED.** The plugin module loaded in the gateway process still has `_RUNS_ROOT` cached from when the junction was active. `Path.resolve()` on Windows follows junctions silently, so `_read_runs_root()` returns the junction-resolved path even after the junction is deleted. In-session `hz_get_metrics` will keep showing the stale `runs_root` until the gateway process is restarted and reloads the plugin module with the junction gone.

```bash
hermes gateway restart
# Then /reset the session to pick up fresh plugin bytecode
```

**Verification after restart — `execute_code` vs in-session comparison:**
If `execute_code` (fresh import) shows the correct E: path but in-session `hz_get_metrics` still shows D: path, the gateway module is stale — restart is the only fix. The module-level `_RUNS_ROOT = Path(HORIZON_CWD) / "data" / "mcp-runs"` is computed once at import time; `.resolve()` on subsequent calls follows whatever the filesystem looked like at that moment.

**Why this matters:** The junction keeps the old path as a valid directory. `Path.resolve()` follows reparse points to the real path — if the junction was present when the plugin module was first imported, every subsequent `_RUNS_ROOT.resolve()` traces through it. Python tracebacks within the MCP subprocess also resolve `__file__` through the junction, polluting error messages with the old D: prefix.

Full session evidence: `references/path-migration-2026-07-05.md`.
...
```bash
ls $HORIZON_HOME/data/mcp-runs/
# Remove any run missing enriched_items.json or both summary files
rm -rf $HORIZON_HOME/data/mcp-runs/<incomplete-run-id>
```
...
- `references/path-migration-2026-07-05.md` — 2026-07-05: D:→E: path migration session — stale paths in 12 skill .md files, workflow correction (sync → replace → branch → test), agent mistakes (do NOT modify plugin or create junctions)
- `references/manual-pipeline-bypass-2026-07-05.md` — 2026-07-05: plugin subprocess broken (gateway Python lacks mcp) → manual JSON-RPC bypass, stdin-closure kills async pipeline, DEEPSEEK_API_KEY missing in subprocess env, partial run recovery
