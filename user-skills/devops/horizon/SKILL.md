...
- `references/subprocess-lifecycle.md` — 2026-06-30: plugin subprocess lifecycle: lazy spawn, reuse, three kill scenarios, subagent isolation, agentmemory comparison. Source-level trace of `_ensure_proc()` / `_kill_proc()` in `__init__.py`.
- `references/fork-maintenance-workflow.md` — 2026-06-30: git workflow for maintaining a personal Horizon fork: remote setup, rebase against upstream, feature branch, push via proxy.
- `references/plugin-architecture-comparison.md` — 2026-06-30: Horizon vs agentmemory plugin architecture — both are thin bridges to external processes, neither injects source code. Covers subprocess lifecycle, Dockerization implications, and why direct import is avoided.
