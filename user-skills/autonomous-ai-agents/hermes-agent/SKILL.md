...
For cross-profile one-shots (e.g. having another profile's persona respond),
use `hermes -p <profile> -z "prompt"` — the `-z` global flag runs a single-turn
query without a subcommand. Full patterns and pitfalls:
`skill_view(name="hermes-agent", file_path="references/cross-profile-communication.md")`
...
Detailed setup guides for specific Hermes features:

- **Vision (auxiliary image analysis):** Configuring non-vision models to analyze images via auxiliary providers (DashScope, Zhipu, Gemini, etc.). See `skill_view(name="hermes-agent", file_path="references/vision-setup.md")`.
- **Desktop & Dashboard:** Building and launching the Web Dashboard (Vite/React) and Electron Desktop app on Windows. See `skill_view(name="hermes-agent", file_path="references/desktop-setup.md")`.
- **Skill Authoring:** Writing SKILL.md files (frontmatter, structure, validation) for in-repo skills. See `skill_view(name="hermes-agent", file_path="references/skill-authoring.md")`.
- **Migration Between Machines:** Auditing, exporting, and restoring a Hermes setup (skills, config, .env, source patches). See `skill_view(name="hermes-agent", file_path="references/migration.md")`.
