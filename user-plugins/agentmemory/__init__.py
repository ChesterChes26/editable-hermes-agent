...
class AgentMemoryProvider(MemoryProvider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._pending_observes: set = set()
    ...
    def initialize(self, session_id: str, **kwargs: Any) -> None:
        ...
        self._pending_observes = set()
        self._observe_lock = threading.Lock()
        if os.environ.get("AGENTMEMORY_REQUIRE_HTTPS") == "1":
            _check_plaintext_bearer_guard(self._base, os.environ.get("AGENTMEMORY_SECRET", ""))

        result = _api(self._base, "session/start", {
            "sessionId": session_id,
            "project": self._project,
            "cwd": self._project,
        })
        logger.warning("DIAG: agentmemory initialize session/start base=%r sid=%s result=%s",
                       self._base, session_id, result)
    ...
    def sync_turn(self, user: str, assistant: str, **kwargs: Any) -> None:
        ...
        logger.info("DIAG: sync_turn session/start base=%r sid=%s result=%s",
                    self._base, sid, "registered" if result else "FAILED")

        # Tracked daemon observe — thread is added to _pending_observes so
        # on_session_end / on_session_switch(reset=True) can flush before
        # ending the session.  try/finally guarantees the thread removes
        # itself from the set even if _api raises an unexpected exception.
        def _do_observe():
            try:
                _api(self._base, "observe", {
                ...
                with self._observe_lock:
                    self._pending_observes.discard(threading.current_thread())

        t = threading.Thread(target=_do_observe, daemon=True)
        with self._observe_lock:
            self._pending_observes.add(t)
        t.start()

    def on_session_end(self, messages: list, **kwargs: Any) -> None:
        # Flush pending observe threads before ending the session server-side.
        # Without this, daemon observe calls race against session/end and
        # observations for the ending session may be silently dropped.
        self._flush_observes()
        _api(self._base, "session/end", {
            "sessionId": kwargs.get("session_id", self._session_id),
        })

    def _flush_observes(self, timeout: float = 30.0) -> None:
        """Wait for all pending observe daemon threads to complete.

        Called at session boundaries (on_session_end, on_session_switch
        with reset=True) to guarantee all observations for the old session
        are committed before the session is ended server-side.
        """
        with self._observe_lock:
            threads = list(self._pending_observes)
        for t in threads:
    ...
    def on_session_switch(
        ...
        **kwargs: Any,
        ...
        Updates cached session_id and calls session/start on the new session
        so subsequent sync_turn / prefetch / system_prompt_block calls target
        the correct session.

        When reset=True (/new, /reset): flushes pending observe threads for
        the old session, calls session/end for the parent, clears accumulated
        per-session state, then registers the new session via session/start.
        """
        if rewound:
            return

        if reset:
            # === RESET: genuinely new conversation ===
            # 1. Flush all pending observe threads for the old session
            self._flush_observes()

            # 2. End old session server-side (defense in depth —
            #    on_session_end from commit_memory_session normally covers
            #    this, but gateway/compression paths may skip it)
            if parent_session_id:
                _api(self._base, "session/end",
                     {"sessionId": parent_session_id})

            # 3. Clear accumulated per-session state
            with self._observe_lock:
                self._pending_observes.clear()

            logger.warning(
                "DIAG: agentmemory RESET old=%s new=%s flushed=%s",
                parent_session_id, new_session_id,
                "yes" if parent_session_id else "no-parent")

        # === START: register new session ===
        ...
        logger.warning("DIAG: agentmemory on_session_switch session/start "
                       "base=%r sid=%s result=%s",
