...
| `/loop [interval] [prompt]` | Run a prompt repeatedly on a time interval while the session stays open. See `/loop` vs `/goal` section below. Alias: `/proactive` |
| `/goal [condition|clear]` | Set a completion condition — Claude keeps working across turns until the condition is met. See `/loop` vs `/goal` section below |
...
### `/loop` vs `/goal` — Two Automation Approaches

Claude Code offers two complementary slash commands for keeping a session running across turns. Pick based on what should trigger the next turn:

| Dimension | `/loop` | `/goal` |
|---|---|---|
| **Driver** | Time interval (or self-paced) | Completion condition |
| **Next turn starts when** | Interval elapses | Previous turn finishes (immediately) |
| **Ends when** | You stop it, or Claude decides work is done | A small fast model (default: Haiku) confirms the condition is met |
| **Min interval** | 1 minute | N/A (fires after each turn) |
| **Version required** | v2.1.72+ | v2.1.139+ |

**`/loop`** — Time-driven polling. Three modes:
- Fixed interval: `/loop 5m check if the deploy finished`
- Dynamic interval (omit interval): Claude self-paces 1min–1hr based on observations
- Maintenance mode (omit prompt): runs built-in maintenance prompt or `.claude/loop.md`
- Can also invoke another command: `/loop 20m /review-pr 1234`

**`/goal`** — Condition-driven autonomous work. Under the hood: a session-scoped prompt-based Stop hook. After each turn, your configured small fast model evaluates the condition against the conversation transcript. If not met, Claude starts another turn immediately. Example:
```
/goal all tests in test/auth pass and the lint step is clean
```
The evaluator reads ONLY what Claude has surfaced in the conversation — it does NOT run commands or read files. Write conditions as something Claude's own output can demonstrate (test results, exit codes, file counts). Max 4,000 characters. Add `or stop after 20 turns` to bound runtime. Works in non-interactive mode: `claude -p "/goal ..."`. Clear early with `/goal clear`. Check status with bare `/goal`.

**When to use which:**
- Use `/loop` for monitoring/polling — checking deploy status, watching PR CI, periodic reminders where you don't know when the work will complete
- Use `/goal` for work with a verifiable end state — migrating a module until tests pass, implementing a design doc until all acceptance criteria hold, clearing an issue backlog
- `/goal` + auto mode together remove both per-tool and per-turn prompts for fully unattended work
- Both are session-scoped. `/loop` jobs survive `--resume` if unexpired (<7 days). `/goal` resets turn count/timer/tokens on resume but carries the condition forward.
